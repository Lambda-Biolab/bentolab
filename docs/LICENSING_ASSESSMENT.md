...
2. ~~**Add a LICENSE file** — currently missing entirely. Recommended: MIT or
   ...
   file. See `LICENSE` and `NOTICE` at the repo root. Apache was preferred over
   MIT for the explicit patent grant, retaliation clause, change-tracking
   requirement, and trademark scope (Section 6) — all of which matter for a
   reverse-engineered library that names third-party hardware.
