...
with `x N` cycle annotation; the active box is highlighted; bottom
...
from .._stages import StageInfo, total_cycle_count
...
def _cycles_label(profile: PCRProfile, box_w: int) -> Text | None:
    if not profile.cycles:
        return None
    total = total_cycle_count(profile)
    # Position the "x N cycles" caption under boxes 1-3 (denat/anneal/extend).
    leading = (box_w + 3) + 1  # box 0 + arrow
    span = (box_w * 3) + 6
    caption = f"<--- x {total} cycles --->"
