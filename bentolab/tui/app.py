...
class BentoLabApp(App):
    ...
    def action_edit_profile(self) -> None:
        ...
        if not name:
            ...
            return
        try:
            path = profile_store.path_for(name)
        except Exception as e:
    ...
    def _refresh_external_diagram(self) -> None:
        ...
        self.diagram.update_stage(stage_at(pane._active_profile, elapsed))
    ...
    def on_run_progressed(self, message: RunProgressed) -> None:
        ...
        if self.status_pane._active_profile is not None:
            active = self.status_pane._active_profile
            self.diagram.update_stage(stage_at(active, message.state.elapsed_seconds))
...
def run() -> None:
    """Entry point for `from bentolab.tui import run`."""
    BentoLabApp().run()
