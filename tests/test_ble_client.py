...
async def test_on_disconnect_schedules_background_reconnect(lab, monkeypatch):
    ...
    lifetime (issue #55). When the BLE link drops, the client
    should schedule a background task that tries to reconnect
    with exponential backoff so a long-running server recovers
    without operator intervention.
    """
    import asyncio

    lab._connected_address = "AA:BB:CC:DD:EE:FF"
    # _on_disconnect runs from bleak's callback thread in production,
    # which uses call_soon_threadsafe to schedule back on our
    # asyncio loop. We need a loop reference for create_task to work
    # in the test -- pytest-asyncio provides one via get_running_loop().
    lab._loop = asyncio.get_running_loop()
    ...
    # Drain the existing callbacks list so the previous test's
    # ``called.append`` lambda doesn't interfere.
    lab._disconnect_callbacks.clear()

    lab._on_disconnect(None)
    assert lab._reconnect_task is not None
    assert not lab._reconnect_task.done()
    # Cancel the task immediately so the test doesn't actually wait.
    task = lab._reconnect_task
    task.cancel()
...
async def test_background_reconnect_fires_callback_on_success(lab, monkeypatch):
    ...
    consumers see a ``reconnected`` event and resume the telemetry
    stream.
    """
    called = []
    ...
    await lab._background_reconnect()
...
async def test_disconnect_cancels_in_flight_reconnect(lab):
    ...
    the in-flight task has been cancelled.
    """
    import asyncio

    assert lab._reconnect_task is None

    async def hang_forever() -> None:
        await asyncio.sleep(3600)

    task = asyncio.create_task(hang_forever())
    lab._reconnect_task = task
    await lab.disconnect()
    # disconnect() cancelled the task under suppress; give the loop
    # a chance to apply the cancel and assert the task is done.
